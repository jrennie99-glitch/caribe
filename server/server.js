...
const ROUTES = [
  ...
  ['GET',  '/api/reserve',      (uid)    => api.reserve(uid),        true ],
  ['POST', '/api/order',        (uid, b) => api.orderCreate(uid, b), true ],
  ['GET',  '/api/orders',       (uid)    => api.orderList(uid),      true ],
  ['POST', '/api/orders/claim', (uid, b) => api.orderClaim(uid, b),  true ],
  ['POST', '/api/orders/update',(uid, b) => api.orderUpdate(uid, b), true ],
  ['POST', '/api/request',      (uid, b) => api.requestMoney(uid, b),true ],
  ['POST', '/api/split',        (uid, b) => api.splitBill(uid, b),   true ],
  ['GET',  '/api/requests',     (uid)    => api.requestsList(uid),   true ],
  ['POST', '/api/requests/pay', (uid, b) => api.payRequest(uid, b),  true ],
  ['POST', '/api/requests/decline',(uid,b)=> api.declineRequest(uid, b), true ],
